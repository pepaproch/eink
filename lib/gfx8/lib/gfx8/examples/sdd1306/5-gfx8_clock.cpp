...
Program: 5-gfx8_clock

Purpose: Testing run-time use of utf8tocp() in gfx8 library

Note:    This program is the same as 3-check_sdd1306_gfx8 except that the ' am" and
         ' pm' strings are not string literals coded for the FreeMono12PtClock code
         page. Instead they remain ASCII strings that are converted to the GFX code
         page 'on the fly' with utf8tocp().
...
GFXfont gfxfont = FreeMono12ptClock;           
...
void setup() {
  ...
  initGfx8charset((uint16_t*) ClockCharset, ClockCharcount, 0x30, 0); // intialize the default gfx8charset_t structure
