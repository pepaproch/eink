...
For UNIX-like systems, compile this source code with make

~$ make -f Makefile8

which will create the executable, fontconvert8, in the same directory.

Derived from but NOT a replacement for Adafruit fontconvert.c 
See <https://sigmdel.ca/michel/program/misc/xxxxxxxxxx> for
...
// Specify which glyph to fetch for 0x7F if that (control) character is 
// in the chartable definition. Three explicit glyphs plus a fourth
// default glyph are possible:
//
#define SPACE_AS_DEL  0                 // " "   0x20 the space character
#define REPLACEMENT_CHARACTER_AS_DEL  1 // "<?>" 0xFFFD the Unicode replacement character
#define APL_QUAD_QUESTION_AS_DEL  2     // "[?]" 0x2370 the APL FUNCTIONAL SYMBOL QUAD QUESTION 

#define DEL_GLYPH REPLACEMENT_CHARACTER_AS_DEL
//
// If DEL_GLYPH is not defined then DEL 0x7F will be used what ever that is
// in the TFT font definition file. In FreeFont TFT files this appears 
// to be 0x2370, the APL quad questions [?]
...
// Choose a chartable header file
...
int main(int argc, char *argv[]) {
  ...
  int first = chartable[0].ucode, last = charcount + first -1;
  int x, y, byte;
  uint16_t code;
  char *fontName, c, *ptr;
  FT_Library library;
  ...
  // Allocate space for font name and glyph table (maximum size 256)
  if ((!(fontName = malloc(strlen(ptr) + 20))) ||
      (!(table = (GFXglyph *)malloc(256 * sizeof(GFXglyph))))) {
...
/****************************************************************
This is where we make the substitution for 0x7F if needed

original code:
    if ((err = FT_Load_Char(face, chartable[i].ucode, FT_LOAD_TARGET_MONO))) {
*****************************************************************/         
    code = chartable[i].ucode;
#if defined(DEL_GLYPH)
    if (code == 0x7F) { 
  #if (DEL_GLYPH == REPLACEMENT_CHARACTER_AS_DEL)
      code = 0x2370;
      strncpy(chartable[i].name, "REPLACEMENT CHARACTER *", NAME_SIZE);
  #elif (DEL_GLYPH == APL_QUAD_QUESTION_AS_DEL)
      code = 0x2370;
      strncpy(chartable[i].name, "APL FUNCTIONAL SYMBOL QUAD QUESTION *", NAME_SIZE);
  #else // DEL_GLYPH == SPACE_AS_DEL
      code = 0x20;
      strncpy(chartable[i].name, " *", NAME_SIZE);
  #endif
    chartable[i].ucode = code;  
    }
#endif
